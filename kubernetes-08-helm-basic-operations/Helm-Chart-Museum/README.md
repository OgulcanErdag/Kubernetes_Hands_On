...
```bash
mkdir my-charts
mv ogulcan-chart-0.1.0.tgz my-charts
...
entries:
  ogulcan-chart:
    - apiVersion: v2
      appVersion: 1.16.0
      created: "2026-03-07T11:59:09.466396276+03:00"
      description: A Helm chart for Kubernetes
      digest: 712c46edcd85b167881bb644d8de4391eee9acd76aabb75fa2f6e53bedd1c872
      name: ogulcan-chart
      type: application
      urls:
        - http://<public ip>:8080/ogulcan-chart-0.1.0.tgz
      version: 0.1.0
generated: "2026-03-07T11:59:09.466104188+03:00"
```
...
```bash
cd my-charts
curl --data-binary "@ogulcan-chart-0.1.0.tgz" http://<public ip>:8080/api/charts
```
...
- Let's see how to maintain the chart version. In `ogulcan-chart/Chart.yaml`, set the `version` value to `0.1.1`and then package the chart.

```bash
helm package ogulcan-chart
mv ogulcan-chart-0.1.1.tgz my-charts
helm repo index my-charts --url http://<public-ip>:8080
```
...
```bash
cd my-charts
curl --data-binary "@ogulcan-chart-0.1.1.tgz" http://<public ip>:8080/api/charts
```
...
- In ogulcan-chart/Chart.yaml, set the `version` value to `0.2.0`and push the chart.

```bash
cd
helm cm-push ogulcan-chart mylocalrepo
```
...
```bash
helm cm-push ogulcan-chart mylocalrepo --version="1.2.3"
```
...
```bash
helm cm-push ogulcan-chart mylocalrepo --version="1.2.4"
```
...
helm repo update
helm search repo mylocalrepo
helm install from-local-repo mylocalrepo/ogulcan-chart
```
...
```bash
helm cm-push ogulcan-chart mylocalrepo --version="1.2.5"
helm repo update
```
...
```bash
helm upgrade from-local-repo mylocalrepo/ogulcan-chart
```
...
```bash
helm upgrade from-local-repo mylocalrepo/ogulcan-chart --version 1.2.4
```
