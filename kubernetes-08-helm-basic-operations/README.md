...
> _Note: If you have a problem with kubernetes cluster, you can use this link for the lesson._
...
```bash
helm create ogulcan-chart
```

- See the files of ogulcan-chart.
...
```bash
rm -rf ogulcan-chart/templates/*
```

- Create a `configmap.yaml` file under the `ogulcan-chart/templates` folder with the following content.

```yaml
apiVersion: v1
kind: ConfigMap
metadata:
  name: ogulcan-chart-config
data:
  myvalue: "ogulcan-chart configmap example"
  course: "DevOps"
```

- Install the ogulcan-chart.

```bash
helm install helm-demo ogulcan-chart
```
...
```bash
NAME: helm-demo
LAST DEPLOYED: Sun Mar  22 11:39:46 2026
NAMESPACE: default
STATUS: deployed
REVISION: 1
TEST SUITE: None
...
```bash
kubectl get cm
kubectl describe cm ogulcan-chart-config
```
...
- Let's create our own values and use them within the template. Update the `ogulcan-chart/values.yaml` as below.

```yaml
course: DevOps
```

- Edit the ogulcan-chart/templates/configmap.yaml as below.
...
```bash
helm install --debug --dry-run mydryrun ogulcan-chart
```

- Install the ogulcan-chart.

```bash
helm install myvalue ogulcan-chart
```
...
```bash
helm install --debug --dry-run setflag ogulcan-chart --set course=AWS
```
...
- Let's try more examples to get more clarity. Update the `ogulcan-chart/values.yaml` as below.
...
- Update the `ogulcan-chart/templates/configmap.yaml` as below.
...
```bash
helm install --debug --dry-run builtin-object ogulcan-chart
```
...
- To add installation notes to your chart, simply create a `ogulcan-chart/NOTES.txt` file. This file is plain text, but it is processed like a template and has all the normal template functions and objects available.

- Let's create a simple `NOTES.txt` file under `ogulcan-chart/templates` folder.
...
```bash
helm install notes-demo ogulcan-chart
```
...
- First, update the `ogulcan-chart/values.yaml` as below.
...
- Next, in `ogulcan-chart/Chart.yaml`, set the `version` value to `0.1.1`and then package the chart.

```bash
helm upgrade notes-demo ogulcan-chart
```
...
```bash
git add .
git commit -m "ogulcan-chart is added"
git push
```
