...
```bash
eksctl create cluster --region us-east-1 --version 1.34 --zones us-east-1a,us-east-1b,us-east-1c --node-type c7i-flex.large --nodes 2 --nodes-min 2 --nodes-max 3 --name cw-cluster
```
...
```bash
kubectl create ingress myshop-ingress --rule="myshop.ogulcan-erdag.de/*=myshop-svc:80" --rule="myshop.ogulcan-erdag.de/account/*=account-svc:80" --class=nginx --annotation="nginx.ingress.kubernetes.io/rewrite-target=/"
```
...
kubectl get ingress
NAME                 CLASS   HOSTS                     ADDRESS                                                                         PORTS   AGE
myshop-ingress   nginx   myshop.ogulcan-erdag.de   afdfe2adcb6934b4abb645258b8f73d2-501976fbe439549f.elb.us-east-1.amazonaws.com   80      70s
```

- To reach the application with `host` name, create a `myshop.ogulcan-erdag.de` record for the address (network load balancer) in the `route53` service.
...
kubectl create ingress myingress \
  --rule="apache.ogulcan-erdag.de/*=mynginx:80" \
  --rule="apache.ogulcan-erdag.de/*=myapache:80" \
  --class=nginx \
...
```bash
NAME        CLASS   HOSTS                                            ADDRESS                                                                         PORTS   AGE
myingress   nginx   nginx.ogulcan-erdag.de,apache.ogulcan-erdag.de   afdfe2adcb6934b4abb645258b8f73d2-501976fbe439549f.elb.us-east-1.amazonaws.com   80      6s
```

- To reach the application with `host` name, create `nginx.ogulcan-erdag.de,apache.ogulcan-erdag.de` records for address (network load balancer) in `route53` service.

- Check the host address.

```bash
curl nginx.ogulcan-erdag.de
curl apache.ogulcan-erdag.de
```
...
```bash
kubectl apply -f ogulcan-pv-claim.yaml
```
...
persistentvolumeclaim/ogulcan-pv-claim   Pending                                      myaws-sc       <unset>                 10s
```
...
kubectl delete -f ogulcan-pv-claim.yaml
```
