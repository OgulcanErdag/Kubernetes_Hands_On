# Hands-on Kubernetes-13-extendend-version : Managing Resources for Containers
...
- Part 5 - Configure Max, Minumum and Default Memory and CPU Requests and Limits for a resource under the a Namespace

- Part 6 - Configure Memory and CPU Quotas for a Namespace

- Part 7 - Configure Quotas for API Objects

- Part 8 - Configure maxLimitRequestRatio
...
> _Note: If you have problem with kubernetes cluster, you can use this link for lesson._
...
- Let's see this. Create yaml file named `ogulcan-deploy.yaml`. Notice that, there is 30 replicas.
...
```bash
kubectl apply -f ogulcan-deploy.yaml
```
...
```bash
kubectl delete -f ogulcan-deploy.yaml
```
...
- Update the ogulcan-deploy yaml and make resources.requests.memory 300Mi. Execute the deployment again.
...
- In our case, t3 medium instance has 2 vCPU's. Update the ogulcan-deploy.yaml as below. We make resources.requests.cpu 500 m.
...
kubectl describe limitrange cpu-limit-range --namespace=resources-limits
```
...
- Let's see this. Create a file and name it `ogulcan-pod.yaml`.
...
```bash
kubectl apply -f ogulcan-pod.yaml
```
...
```bash
kubectl delete -f ogulcan-pod.yaml
```

### What if we specify a Container's limit, but not its request?

- Update the ogulcan-pod.yaml. This time we will add `resources.limits`, but not `resources.requests`.
...
### What if we try to specify a Container's limit more than maximum limit?

- Update the ogulcan-pod.yaml. This time we will increase the `resources.limits` more than maximum limit.
...
```bash
Error from server (Forbidden): error when creating "ogulcan-pod.yaml": pods "default-cpu" is forbidden: maximum cpu usage per Container is 1500m, but limit is 2
```

### What if we specify a Container's request, but not its limit?

- Update the ogulcan-pod.yaml. This time we will add `resources.requests` , but not `resources.limits`.
...
### What if we specify a Container's request, more than default limit?

- Update the ogulcan-pod.yaml. This time we will increase `resources.requests` more than `default limit`.
...
```bash
The Pod "default-cpu" is invalid: spec.containers[0].resources.requests: Invalid value: "1100m": must be less than or equal to cpu limit of 1
```
...
- Let's update the `ogulcan-pod.yaml` like below. The Container does not specify a memory request and limit.
...
- Update the ogulcan-pod.yaml. The Container specifies a memory limit, but not a request.
...
- Update the ogulcan-pod.yaml like below. The Container specifies a memory request, but not a limit.
...
- Let's create a pod. Update ogulcan-pod.yaml as below.
...
- We will attempt to create a second Pod. Create a file and name it `ogulcan-pod-2.yaml`.
...
```bash
kubectl apply -f ogulcan-pod-2.yaml
```
...
```
Error from server (Forbidden): error when creating "ogulcan-pod-2.yaml": pods "quota-mem-cpu-2" is forbidden: exceeded quota: mem-cpu, requested: requests.memory=700Mi, used: requests.memory=600Mi, limited: requests.memory=1Gi
```
...
```bash
kubectl delete -f ogulcan-pod.yaml
kubectl delete -f ogulcan-pod-2.yaml
```
...
- Create a file named `ratio-range.yaml`.
...
```bash
kubectl apply -f ratio-range.yaml
kubectl describe limitrange cpu-memory-resource-constraint
```

- Create a file named `pod-ratio.yaml`.
...
```bash
kubectl apply -f ratio-pod.yaml
```

- We will get an output like below.

```bash
Error from server (Forbidden): error when creating "ratio-pod.yaml": pods "pod-ratio" is forbidden: [maximum cpu usage per Container is 1, but limit is 500, cpu max limit to request ratio per Container is 2, but provided ratio is 2500.000000]
```

- This time we will try for memory. Update the `pod-ratio.yaml` file as below.
...
```bash
Error from server (Forbidden): error when creating "ratio-pod.yaml": pods "pod-ratio" is forbidden: [minimum memory usage per Container is 100Mi, but request is 200m, memory max limit to request ratio per Container is 1500m, but provided ratio is 2000.000000]
```
