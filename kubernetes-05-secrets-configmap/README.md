...
The purpose of this hands-on training is to give students the knowledge of Kubernetes Secrets and config-map
...
- Secrets can contain user credentials required by Pods to access a database. For example, a database connection string consists of a username and a password. You can store the username in a file ./username.txt and the password in a file ./password.txt on your local machine.

```bash
# Create files needed for the rest of the example.
echo -n 'admin' > ./username.txt  # "-n " do not output
echo -n '1f2d1e2e67df' > ./password.txt
```

- The kubectl create secret command packages these files into a Secret and creates the object on the API server. The name of a Secret object must be a valid DNS subdomain name. Show types of secrets with opening: (Kubernetes Secret Types)[https://kubernetes.io/docs/concepts/configuration/secret/]
[https://kubernetes.io/docs/reference/generated/kubectl/kubectl-commands#-em-secret-em-] [https://kubernetes.io/docs/reference/kubernetes-api/config-and-storage-resources/secret-v1/]

```bash
k create secret generic --help
kubectl create secret generic db-user-pass --from-file=./username.txt --from-file=./password.txt # --from-literal=key1=supersecret
```
...
k create secret generic my-secret --from-file=/home/ubuntu/Lesson # from diroctory
```
>Note:
...
```bash
kubectl describe secrets/db-user-pass # k describe secret db-user-pass
...
- Firstly, we get the parameters as plain environment variables.
...
- Inside a container that consumes a secret in an environment variable, the secret keys appear as normal environment variables containing the base64 decoded values of the secret data. This is the result of commands executed inside the container from the example above:

- Enter into pod and type the following command.
...
root@secret-env-pod:/data# echo $SECRET_PASSWORD
1f2d1e2e67df
root@secret-env-pod:/data# printenv # all env
root@secret-env-pod:/data# env
exit
```
...
- ConfigMap helps to make configurations easier to change and manage, and prevents hardcoding configuration data in Pod specifications.

- ConfigMaps are useful for storing and sharing non-sensitive, unencrypted configuration information.

- For the showcase, we will select a simple application that displays a message like this.

```text
Hello, Clarusway!
```

- We will parameterize the "Hello" portion in some languages.
...
```bash
docker run -p 80:8888 -d ondiacademy/demo:hello-config-env
docker ps
# See the app on browser  
http://<master-public-IP>:80
docker rm -f e54(container-id)
```

```bash
docker run -p 80:8888 -d -e GREETING=selam ondiacademy/demo:hello-config-env
docker ps
# See the app on browser  
http://<master-public-IP>:80
docker rm -f e54(container-id)
...
- This time, we will expose configuration data to the container’s environmental variables. And, we will create a `ConfigMap` and use the `greeting` key-value pair as in the `deployment.yaml` file.

## Create and use ConfigMaps with the `kubectl create configmap` command
...
We will start with the third option. We have just one parameter. Greet with "Hola" in Spanish.
...
- Explain the important parts in the `ConfigMap` file contents.
...
## Creating a ConfigMap with template
...
- In case you are using envFrom  instead of env  to create environmental variables in the container, the environmental names will be created from the ConfigMap's keys. If a ConfigMap  key has invalid environment variable name, it will be skipped but the pod will be allowed to start. 

Modify the configmap.yaml file:
...
- You can compare with the previous `deployment.yaml` file.
...
NAME           TYPE           CLUSTER-IP       EXTERNAL-IP   PORT(S)        AGE
demo-service   NodePort       10.110.195.109   <pending>     80:30001/TCP   17s
...
- Check the environment variables and see that all the environment variables in the configmap are injected into the pod. 
...
```bash
Welcome to the Kubernetes Lessons.
```

- Let's create our configmap from the `content` file.
    ...
    spec:
      ...
      - name: nginx-config-volume
        configMap:
          ...
          items:
          - key: content
            path: index.html # https://kubernetes.io/docs/concepts/storage/volumes/#configmap
```

- Volume and volume mounting are common ways to place config files inside a container. We are selecting the `content` key from `nginx-config` ConfigMap and put it inside the container at path `/usr/share/nginx/html/` with the name `index.html`.

- Apply and run all the configurations as follows:
...
- Notice that the names of files inside the test folder are the `keys of nginx-secret` object, and the content of files are `values of nginx-secret` object.
