...
- Create yaml file named `ogulcan-deploy.yaml` and explain its fields.
...
- Update the ogulcan-deploy.yaml as below.
...
- Now, we will update ogulcan-deploy.yaml as below. We just add `nodeSelector` field.
...
- We will update ogulcan-deploy.yaml as below. We will add an affinity field instead of nodeSelector.
...
- This time we will test `preferredDuringSchedulingIgnoredDuringExecution`. Update ogulcan-deploy.yaml as below. This time, we change `requiredDuringSchedulingIgnoredDuringExecution` with `preferredDuringSchedulingIgnoredDuringExecution`.
...
- create yaml file and named `ogulcan-db.yaml`.
...
```bash
kubectl apply -f ogulcan-db.yaml
```
...
- Create yaml file named `ogulcanshop-deploy.yaml`.
...
```bash
kubectl apply -f ogulcanshop-deploy.yaml
```

- List the pods and notice that the ogulcanshop pods are assigned to the same nod with ogulcan-db pod.
...
```bash
kubectl delete -f ogulcanshop-deploy.yaml
kubectl delete -f ogulcan-db.yaml
```
