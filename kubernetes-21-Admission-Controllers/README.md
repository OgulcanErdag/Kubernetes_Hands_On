...
- Let's check it. Create a pod on the `ogulcan` namespace.

```bash
kubectl run nginx-pod --image nginx -n ogi
Error from server (NotFound): namespaces "ogi" not found
```
...
- Wait for restarting of the kube-apiserver. Firstly check the namespaces and notice that there isn't the `ogi` namespace, and try to create the pod again.
...
- This time the pod was created. And check the namespaces and see that there is the `ogi` namespace.
...
```bash
kubectl -n ogi get pod nginx-pod -o yaml | grep -i serviceaccount
