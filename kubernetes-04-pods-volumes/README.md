...
The purpose of this hands-on training is to give students the knowledge of Kubernetes Volumes.
...
- View information about the `PersistentVolumeClaim` and show that the `pv-claim-2g` is bound to the PersistentVolume `pv-3g`. Notice that the capacity of the pv-claim-2g is 3Gi.
...
- View information about the `PersistentVolume's` and `PersistentVolumeClaim's` and show that the status of `pv-claim-7g` is `pending` and the status of pv-6g is available.
...
- Log in to the kube-master ec2-instance again and connect to the nginx-pod. See that the test folder and content are there.
